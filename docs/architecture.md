...
1. **Frontend (Client-Side)**: React 19 application running in the browser. It handles state management, global fuzzy search via Fuse.js, and strictly executes all cryptographic operations (`age-encryption` and `fflate`) in memory. 
...
sequenceDiagram
    ...
    PassphraseGate-->>User: Grant Access & Render FolderView
...
2. The `GlobalSearchModal` utilizes `Fuse.js` to execute real-time, space-insensitive fuzzy matching across this decrypted memory cache.
